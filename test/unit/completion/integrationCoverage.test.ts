...
describe("integration-promoted completion and hover coverage", () => {
  it("keeps version-specific mode completion and hover behavior in unit coverage", async () => {
    const content = "defaults\n    mode h\n";
    const cursor = "    mode h".length;

    expect(await labels(content, 1, cursor, "3.2")).toContain("http");
    expect(await labels(content, 1, cursor, "3.2")).not.toContain("haterm");
    expect(hoverText(content, 1, 7, "3.2")).not.toContain("haterm");

    expect(await labels(content, 1, cursor, "3.4")).toEqual(
      expect.arrayContaining(["haterm", "http"]),
    );
    expect(hoverText(content, 1, 7, "3.4")).toContain("haterm");
  });

  it("covers integration-level section and symbol-reference completions", async () => {
    expect(await labels("", 0, 0, "3.2")).toEqual(
  ...
  it("covers line-option and action-family completions without Electron", async () => {
    expect(
      await labels(
        "frontend web\n    bind 127.0.0.1:80 ",
        1,
        "    bind 127.0.0.1:80 ".length,
        "3.2",
      ),
  ...
  it("covers file-backed basic-check hover and completion paths", async () => {
    ...
    expect(
      (await labels(content, optionLine, "\t".length, "3.2")).some((label) =>
