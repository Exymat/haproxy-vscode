...
describe("entryPointDiagnostics", () => {
  ...
  it("does not inherit bind settings from following defaults", () => {
    const document = createDocument("frontend web\n    mode http\ndefaults\n    bind :80");
    const parsed = parseDocument(document);
    ...
    expect(
      entryPointWithoutBindDiagnostics(document, parsed, entryCtx).some(
        (diagnostic) => diagnostic.code === "no-bind-entry-point",
      ),
    ).toBe(true);
  });

  it("keeps defaults sections without inherited bind state", () => {
    const document = createDocument("defaults\n    mode http");
    const parsed = parseDocument(document);
    const blocks = buildSectionBlocks(parsed, schema);
    const memo = new Map<number, boolean>();
  ...
  it("falls back to raw section labels and unnamed section names", () => {
    ...
    const customCtx = {
      entryPointSections: entryPointSectionSet(customSchema),
      bindDetectKeywords: bindDetectKeywordSet(customSchema),
      schema: customSchema,
    };
    const document = createDocument("frontend\n    mode http");
    const parsed = parseDocument(document);
    const [diagnostic] = entryPointWithoutBindDiagnostics(document, parsed, customCtx);

    expect(diagnostic?.message).toContain("frontend");
  });
