...
describe("extensionBundle", () => {
  ...
  it("formats Error and non-Error bundle failure messages", () => {
    expect(bundleErrorMessage(new Error("typed failure"))).toBe("typed failure");
    expect(bundleErrorMessage("string failure")).toBe("string failure");
  });

  it("throws a clear workspace schema error when the safe bundle loader cannot recover", async () => {
    vi.spyOn(schema, "loadSchemaAsync").mockRejectedValue(new Error("schema unavailable"));
    vi.spyOn(languageData, "loadLanguageDataAsync").mockResolvedValue(fixture.languageData);
    const service = createExtensionBundleService(mockExtensionContext() as never);

    await expect(service.resolveWorkspaceSchema(undefined)).rejects.toThrow(
      "HAProxy schema bundle is unavailable",
    );
  });

  it("reports bundle errors only once until reset or dispose", () => {
    const service = createExtensionBundleService(mockExtensionContext() as never);

    service.reportBundleError("first");
    service.reportBundleError("second");
    expect(window.showErrorMessage).toHaveBeenCalledTimes(1);
    expect(window.showErrorMessage).toHaveBeenLastCalledWith(
      "HAProxy extension failed to load schema: first",
    );

    service.resetErrorReporting();
    service.reportBundleError("after reset");
    expect(window.showErrorMessage).toHaveBeenCalledTimes(2);
    expect(window.showErrorMessage).toHaveBeenLastCalledWith(
      "HAProxy extension failed to load schema: after reset",
    );

    service.dispose();
    service.reportBundleError("after dispose");
    expect(window.showErrorMessage).toHaveBeenCalledTimes(3);
    expect(window.showErrorMessage).toHaveBeenLastCalledWith(
      "HAProxy extension failed to load schema: after dispose",
    );
  });
});
