...
function collectConfiguredReferences(
  ...
  for (const pattern of patterns) {
    ...
    if (matchLength === 0 || line.tokens.length <= pattern.target_token_index) {
      continue;
    }
    for (let start = 0; start + matchLength <= line.tokens.length; start += 1) {
      let matched = true;
      for (let i = 0; i < matchLength; i += 1) {
        if (line.tokens[start + i]?.text.toLowerCase() !== pattern.match_tokens[i]?.toLowerCase()) {
          matched = false;
          break;
        }
      }
      ...
      const token = line.tokens[targetIndex];
      if (!token) {
        continue;
      }
...
function resolveStatementRuleSymbol(
  ...
  for (const pattern of schema.reference_patterns ?? []) {
    ...
    for (let start = 0; start + matchLength <= line.tokens.length; start += 1) {
      ...
      const targetIndex = start + pattern.target_token_index;
      const token = line.tokens[targetIndex];
      if (!token || tokenIndex !== targetIndex) {
        continue;
      }
      return {
        kind: pattern.reference_kind as SymbolKind,
